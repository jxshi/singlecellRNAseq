# Step 8.2: assign dendritic cell subtypes
...
dc_dir <- file.path(base_dir, "DCs")
setwd(dc_dir)

sce_path <- file.path(dc_dir, "DCs.sce.sub.harmony.Rds")
...
dc_markers <- list(
  ...
  pDC = c("Bst2", "Siglech", "Il3ra", "Tlr7", "Tlr9"),
  T_cells = c("Cd3d", "Cd3e", "Cd4", "Cd8a", "Cd8b1")
)

save_dotplot(sce_sub, unique(unlist(dc_markers)), "DotPlot_DCs_marker_panels_by_subtype.pdf", title = "Marker overview by subtype")
save_dotplot(
  sce_sub,
  unique(unlist(dc_markers)),
  "DotPlot_DCs_marker_panels_by_cluster.pdf",
  title = "Marker overview by cluster"
)

new_labels <- data.frame(
  ClusterID = 0:9,
  newsubcelltype = c(
    "moDC", "cDC2", "cDC1", "pDC", "migratoryDC", "moDC", "CD8 T cells", "migratoryDC", "Th1", "pDC"
  )
)

main_labels <- data.frame(
  ClusterID = 0:9,
  maincelltype = c("DCs", "DCs", "DCs", "DCs", "DCs", "DCs", "Unknown", "DCs", "Unknown", "DCs")
)
...
sce_dc <- sce_sub
saveRDS(sce_dc, file.path(base_dir, "sce.DCs_w_correction.Rds"))

combined_colors <- c(
  "#BC3C29", "#0072B5", "#E18727", "#008B45", "#5F559B", "#6F99AD", "#A20056", "#EE4C97", "#3B4992", "#008280", "#631879", "#C70039", "#FFDC91"
)

fig_dir <- file.path(base_dir, "graphs", "DCsFigures")
dir.create(fig_dir, showWarnings = FALSE, recursive = TRUE)
setwd(fig_dir)

Idents(sce_dc) <- sce_dc$newsubcelltype
sce_plot <- subset(sce_dc, idents = setdiff(levels(Idents(sce_dc)), c("CD8 T cells", "Th1")))
