...
sceList <- SplitObject(sce, split.by = "group")
  ...
  if(T) {
    ...
    saveRDS(sce.sub,file=paste0("cyclingT",".sce.sub.harmony.Rds"))
  }
  ...
  if(T) {
    ...
    pro='_rna'
    write.csv(sce.markers,file = paste0('cyclingT', pro,"_subcluster.markers.csv"))
    
    saveRDS(sce.markers, file = paste0('cyclingT', pro,"_subcluster.markers.Rds"))
    
    library(dplyr)
    
    top10 <- sce.markers %>% group_by(cluster) %>% top_n(10, avg_log2FC)
    DoHeatmap(sce.sub,top10$gene,size=3)
    ggsave(filename=paste0('cyclingT', pro,'_sce.markers_heatmap.pdf'),width = 14,height = 20)
    ...
    p
    ggsave(plot=p, filename=paste0('cyclingT', pro,'DotPlot_check_top10_markers_by_clusters.pdf'), device=cairo_pdf,width = 18,height = 24)
    
    library(dplyr)
    top6 <- sce.markers %>% group_by(cluster) %>% top_n(6, avg_log2FC)
    DoHeatmap(sce.sub,top6$gene,size=3)
    ggsave(paste0('cyclingT', pro,'DoHeatmap_check_top6_markers_by_clusters.pdf'),width = 18,height = 12)
    # DoHeatmapPlot(sce,"batch",top3)
    p <- DotPlot(sce.sub, features = unique(top6$gene), assay='RNA')  + coord_flip()
    p
    ggsave(plot=p, filename=paste0('cyclingT', pro,'DotPlot_check_top6_markers_by_clusters.pdf'),device=cairo_pdf,width = 18,height = 12)
    
    library(dplyr)
    top3 <- sce.markers %>% group_by(cluster) %>% top_n(3, avg_log2FC)
    DoHeatmap(sce.sub,top3$gene,size=3)
    ggsave(paste0('cyclingT', pro,'DoHeatmap_check_top3_markers_by_clusters.pdf'),width = 18,height = 12)
    # DoHeatmapPlot(sce,"batch",top3)
    p <- DotPlot(sce.sub, features = unique(top3$gene), assay='RNA')  + coord_flip()
    p
    ggsave(plot=p, filename=paste0('cyclingT', pro,'DotPlot_check_top3_markers_by_clusters.pdf'),device=cairo_pdf,width = 18,height = 12)
    
  }

# CD4 and CD8
genes_to_check <- c("CD4", "CD8A",'CD3D', 'CD3E', 'TRAC', 'IL7R','CD44','IL2RA','FOXP3',"CTLA4")
...
p
ggsave(p, filename = paste0('cyclingT',"_Dotplot_plot.pdf"), device = cairo_pdf, width = 5, height = 10)
...
if(T) {
  ...
  subcelltype=data.frame(ClusterID=0:14, subcelltype='na')
  
  subcelltype[subcelltype$ClusterID %in% c( 0),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 1),2]='CD8_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 2),2]='CD8_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 3),2]='CD8_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 4),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 5),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 6),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 7),2]='CD8_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 8),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c( 9),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c(10),2]='CD8_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c(11),2]='gdT-proliferating'
  subcelltype[subcelltype$ClusterID %in% c(12),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c(13),2]='CD4_T-proliferating'
  subcelltype[subcelltype$ClusterID %in% c(14),2]='gdT-proliferating '
...
if(T) {
  ...
  sce <- readRDS(paste0('cyclingT', ".sce.sub.harmony.Rds"))
  # write cell type per grouop
  cltyPerSubroup <- table(sce@meta.data$seurat_cluster, sce@meta.data$orig.ident)
  write.table(cltyPerSubroup, paste0('cyclingT', "celltype_per_group.tsv"), quote = F, sep = "\t", row.names = T)
  
  # Plot proportions of different celltypes per cluster
  p <- SubPropPlot(sce, "batch") + coord_flip()
  p
  ggsave(p, filename = paste0('cyclingT',"_subgroup_celltype_proportion_plot.pdf"), device = cairo_pdf, width = 10, height = 3)
}
...
if(T) {
  ...
  p
  ggsave(plot=p, filename=paste0('cyclingT', pro,'DotPlot_check_interested_markers_by_clusters_MonoMacro_cells.pdf'), device=cairo_pdf,width = 16,height = 24)
  ...
  p
  ggsave(plot=p, filename=paste0('cyclingT', pro,'DotPlot_univ_mhc_by_clusters.pdf'), device=cairo_pdf,width=9,height=10)
  ...
  p
  ggsave(plot=p, filename=paste0('cyclingT', pro,'DotPlot_interferon_markers_by_clusters.pdf'), device=cairo_pdf,width=9,height=10)
  ...
  p
  ggsave(plot=p, filename=paste0('cyclingT', pro,'DotPlot_fcReceptor_markers_by_clusters.pdf'), device=cairo_pdf,width=9,height=10)
